 ...
 * @since  1.6
 */
class PlgUserProfile_association extends JPlugin
{
	/**
	 * Date of birth.
	 *
			...
			if (!isset($data->profile) && $userId > 0)
			{
				...
				$db->setQuery(
					'SELECT profile_key, profile_value FROM #__user_profiles'
						. ' WHERE user_id = ' . (int) $userId . " AND profile_key LIKE 'profile_association.%'"
						. ' ORDER BY ordering'
				);
				...
				$data->profile_association = array();

				foreach ($results as $v)
				{
					$k = str_replace('profile.', '', $v[0]);
					$data->profile_association[$k] = json_decode($v[1], true);

					if ($data->profile_association[$k] === null)
					{
						$data->profile_association[$k] = $v[1];
					}
				}
			}
	...
	public function onContentPrepareForm($form, $data)
	{
		...
		{
			$form->setFieldAttribute('first_name', 'description', 'PLG_USER_PROFILE_FILL_FIELD_DESC_SITE', 'profile');
			$form->setFieldAttribute('last_name', 'description', 'PLG_USER_PROFILE_FILL_FIELD_DESC_SITE', 'profile');
			$form->setFieldAttribute('born_at', 'description', 'PLG_USER_PROFILE_FILL_FIELD_DESC_SITE', 'profile');
			$form->setFieldAttribute('born_at_state', 'description', 'PLG_USER_PROFILE_FILL_FIELD_DESC_SITE', 'profile');
			...
			$form->setFieldAttribute('address', 'description', 'PLG_USER_PROFILE_FILL_FIELD_DESC_SITE', 'profile');
			$form->setFieldAttribute('number', 'description', 'PLG_USER_PROFILE_FILL_FIELD_DESC_SITE', 'profile');
			...
			$form->setFieldAttribute('fiscal_code', 'description', 'PLG_USER_PROFILE_FILL_FIELD_DESC_SITE', 'profile');
			$form->setFieldAttribute('country', 'description', 'PLG_USER_PROFILE_FILL_FIELD_DESC_SITE', 'profile');
			$form->setFieldAttribute('identity_document_type', 'description', 'PLG_USER_PROFILE_FILL_FIELD_DESC_SITE', 'profile');
			$form->setFieldAttribute('identity_document_authority', 'description', 'PLG_USER_PROFILE_FILL_FIELD_DESC_SITE', 'profile');
			$form->setFieldAttribute('numer_document', 'description', 'PLG_USER_PROFILE_FILL_FIELD_DESC_SITE', 'profile');
			$form->setFieldAttribute('identity_document_issued_date', 'description', 'PLG_USER_PROFILE_FILL_FIELD_DESC_SITE', 'profile');
			$form->setFieldAttribute('document_expiration_date', 'description', 'PLG_USER_PROFILE_FILL_FIELD_DESC_SITE', 'profile');
			$form->setFieldAttribute('profession', 'description', 'PLG_USER_PROFILE_FILL_FIELD_DESC_SITE', 'profile');
			$form->setFieldAttribute('googleplus', 'description', 'PLG_USER_PROFILE_FILL_FIELD_DESC_SITE', 'profile');
			$form->setFieldAttribute('linkedin', 'description', 'PLG_USER_PROFILE_FILL_FIELD_DESC_SITE', 'profile');
	...
	public function onUserAfterSave($data, $isNew, $result, $error)
	{
		...
		if ($userId && $result && isset($data['profile_association']) && count($data['profile_association']))
		{
			try
			{
				$db = JFactory::getDbo();
				...
				foreach ($data['profile_association'] as $k => $v)
				{
					...
					$tuples[] = '(' . $userId . ', ' . $db->quote('profile_association.' . $k) . ', ' . $db->quote(json_encode($v)) . ', ' . ($order++) . ')';
				}
			...
			try
			{
				...
				$db->setQuery(
					'DELETE FROM #__user_profiles WHERE user_id = ' . $userId
						. " AND profile_key LIKE 'profile_association.%'"
				);
